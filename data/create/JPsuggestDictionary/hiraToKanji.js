...
const translateHiraToKanji = async (targetKeywords) => {
  const kanjiRegex =
    /([\u{3005}\u{3007}\u{303b}\u{3400}-\u{9FFF}\u{F900}-\u{FAFF}\u{20000}-\u{2FFFF}][\u{E0100}-\u{E01EF}\u{FE00}-\u{FE02}]?)/mu; // 漢字が一文字でも含まれているのかどうかの正規表現
...
const createHiraToKanjiData = async (keywordsSource) => {
  ...
  for (const oneFoodData of keywordsSource) {
    const targetKeywords = oneFoodData['keywords'].filter((oneKeyword) =>
      hiraRegex.test(oneKeyword)
    );
    let kanjis = [];
    if (targetKeywords.length > 0) {
      kanjis = await translateHiraToKanji(targetKeywords);
    }

    resKanji.push({
      ...oneFoodData,
      addedKeywords: targetKeywords.length > 0 ? kanjis[0].slice(0, 2) : []
    });
  }
