...
const getItemNoAndFoodNameArr = (GASdata) => {
  const ItemNoAndFoodNameArr = GASdata.map((oneFoodData) => ({
    ItemNo: oneFoodData['ItemNo'],
    FoodName: oneFoodData['FoodName']
  }));
...
const getFoodNameToKeywords = (ItemNoAndFoodNameArr) => {
  const unNeedSpell =
    /＜|＞|［|］|（|）|二次品目|副品目|一般用|その他|低カロリータイプ|電子レンジ調理|/g;
  const keywordsObj = ItemNoAndFoodNameArr.map((oneFoodData) => {
    const foodNameDeletedUnNeedSpell = oneFoodData['FoodName']
      .replace(unNeedSpell, '')
      .replace('　　', '　');
...
const combineKeywords = (keywordsSource, keywordAddedKanji, keywordsAddedHira) => {
  return keywordsSource.map((oneFoodData, index) => {
    ...
    const kanjiKeyword =
      keywordAddedKanji[index]['addedKeywords'] === undefined
        ? []
        : keywordAddedKanji[index]['addedKeywords'];
    const hiraKeyword =
      keywordsAddedHira[index]['addedKeywords'] === undefined
        ? []
        : keywordsAddedHira[index]['addedKeywords'];
...
const createNewJSON = async () => {
  ...
  const ItemNoAndFoodNameArr = getItemNoAndFoodNameArr(GASdata),
    ...
    combinedKeywordSource = await combineKeywords(
      keywordsSource,
      keywordAddedKanji,
      keywordsAddedHira
    );
